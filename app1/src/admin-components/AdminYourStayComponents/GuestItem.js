...
function GuestItem({occupant}) {
  ...
  useEffect(()=>{
    const prom=new Promise((resolve,reject)=>{
      axios.get("https://ghanahomestayserver.onrender.com/current-resident/guests/"+occupant.application_id+"/"+occupant.id).then((response)=>{
        console.log(response)
        setGuests(response.data.guests)
        axios.get("https://ghanahomestayserver.onrender.com/current-resident/restricted-individuals/"+occupant.application_id+"/"+occupant.id).then((response1)=>{
          console.log("\n\n")
